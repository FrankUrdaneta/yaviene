;(function (undefined) {
    ...
    TtxH5Video.prototype = {
        ...
        startListenByUrl: function (url, type) {
            ...
            if (false) {
                ...
                listenOptions = {
                    ...
                    customType: {
                        ttxplayer: function (video, player) {
                            player.ttxplayer = new Cmsv6Player(null, this.soundDomId);
                            player.ttxplayer.play(url, {
                                isLive: true,
                                hasVideo: false,
                                hasAudio: true,
            ...
            if (type === 1) {
                ...
                if (!that.playVideosTimer[index]) {
                    that.playVideosTimer[index] = setInterval(function () {
                        ...
                        if (that.options.isTtx) {
                            if (that.playVideos[index].ttxplayer.playFlag) {
                                speedStr = '';
                            }
        ...
        setBufferTime: function (index, bufferTime, bufferTimeMax) {
            if (this.options.isTtx) {
                if(bufferTime <= 1){
                    bufferTime = 0.2;
                }
        ...
        setTalkParam: function () {
            if (this.options.lang == 'zh-cn') {
                alert('h5播放器暂时不支持对讲功能，请使用flash播放器')
            } else {
                alert('h5 player does not support intercom function temporarily, please use flash player')
            }
        },
